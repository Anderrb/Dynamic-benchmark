...
class Egg( object ):
    ...
    def set_dir( self ):
        self.dir = os.path.realpath( os.path.join( galaxy_dir, 'eggs' ) )
...
def require( req_str ):
    ...
    try:
        ...
        from galaxy.eggs.scramble import HBScrambleCrate
        c = HBScrambleCrate( None )
        try:
            return c[req.project_name].require()
        except KeyError:
        # not a hyperbrowser-owned dependency
