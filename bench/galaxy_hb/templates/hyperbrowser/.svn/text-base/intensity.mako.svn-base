...
<%def name="title()">Create intensity track</%def>
<%def name="head()">
...
%if hyper._userHasFullAccess(galaxy.getUserName()):    
    ...
    <label>Create intensity track as: <select name="create_as" onchange="with($('#input_name')){if(this.selectedIndex==1)show();else hide()}">
...
%endif
    ...
    <p><input id="start" type="submit" value="Create track" ${_disabled(valid, True)}></p>
...
<%def name="toolHelp()">
The Genomic HyperBrowser only does hypothesis testing on up to two tracks at at time. In order to take any confounding tracks into account, the concept of intensity tracks were invented. This tool is used to create such tracks.
...
<hr class='space'>
<b>Extract options</b>
<p>Define which regions of the track where you want to create an intensity track, e.g. where you want to do the analysis. There are several options:
<ul>
...
<hr/ class='space'>
<a href='${h.url_for("hyper?mako=intensity&tool_id=hb_intensity&setparams=1&dbkey=sacCer1&numctrltracks=2&track1=Genes%20and%20gene%20subsets%3AExon%20boundaries%3ALefts&track2=Sequence%3AGC%20content%3ASliding%20window%3ALeft%20100bps%20triangular&track3=Sequence%3AGC%20content%3ASliding%20window%3ARight%20100bps%20triangular&method=__chrs__&__chrs__=*&sepFilePrRegion=1")}'>Demo</a>
</%def>
