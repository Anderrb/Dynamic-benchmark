...
if clusterCase == "use refTracks" :
	for i in range(len(referenceTracks)):
	    referenceTracks[i] = control.getTrackElement('reftrack' + str(i+1), 'refTrack ' + str(i+1),True)
...
<%def name="title()">Cluster tracks</%def>
<%def name="head()">
...
<fieldset><legend>cluster tracks</legend>
...
<p>
    ...
    <input type="button" id="newtrackbtn" value="Add another clustertrack" onclick="with(form){numclustertracks.value ++;action='';submit();}">
</p>
</fieldset>
<p>
    <input type="radio" name="clusterCase" value="use refTracks" ${'checked' if (p.has_key('clusterCase') and p.get("clusterCase")=="use refTracks") else ''} onchange="with(form){action='';submit();}">Use reference tracks?<br>
    <input type="radio" name="clusterCase" value="use pair distance" ${'checked' if (p.has_key('clusterCase') and p.get("clusterCase")=="use pair distance") else ''} onchange="with(form){action='';submit();}">Direct distance between a pair of tracks<br>
    <input type="radio" name="clusterCase" value="self feature" ${'checked' if (p.has_key('clusterCase') and p.get("clusterCase")=="self feature") else ''} onchange="with(form){action='';submit();}">Construct feature vector based on the track itself<br>
...
<fieldset><legend>reftrack & options</legend>
%for ti in range(len(referenceTracks)):
    ...
    %if (p.has_key('track1') and clusterTracks[0].selected() and p.has_key('reftrack'+str(ti+1)) and referenceTracks[ti].selected()):
		...
		%if len(validFeature.keys()) == 0 :
			There is not any valid feature for this refTrack and clusterTrack.<br>
		...
		Choose feature : 
		%if len(validFeature.keys()) == 0 :
			There is not any valid feature for this refTrack. <br>
		%else :
		...
		%endif
		Expand this refTrack, and use as a new refTrack :
...
<p>
    ...
    <input type="button" id="newreftrackbtn" value="Add reftrack" onclick="with(form){numreferencetracks.value ++;action='';submit();}">
</p>
</fieldset>
...
%elif (p.has_key('clusterCase') and p.get("clusterCase") == "self feature") :
    <fieldset><legend>self feature, Options</legend>
    %if (p.has_key('track1') and p.has_key('track2') and clusterTracks[0].selected() and clusterTracks[1].selected()) :
    	<%
		genome = p.get('dbkey')
...
<fieldset><legend>Clustring method & options</legend>
