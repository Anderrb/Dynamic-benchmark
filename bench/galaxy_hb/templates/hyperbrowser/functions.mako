...
<%def name="select(name, opts, value, label = None)">
    <p><label>${label if label else name}
    <select name="${name}" onchange="reloadForm(form, this)">
        %for o in range(len(opts)):
            <option ${'selected' if value == opts[o] else ''}>${opts[o]}</option>
        %endfor
    </select>
    </label></p>
</%def>

<%def name="history_select(control, name, opts, value, label = None)">
    <p><label>${label if label else name}
    <select name="${name}" onchange="reloadForm(form, this)">
        ${opts[0]}
    </select>
...
<%def name="checkbox(name, opts, value, label = None)">
    <label><input onchange="reloadForm(form, this)" id="${name}" name="${name}" type="checkbox" value="True" ${'checked=checked' if value else ''}> ${label}</label>
</%def>

<%def name="multichoice(name, opts, values, label = None)">
...
<%def name="multihistory(name, opts, values, label = None)">
    <fieldset><legend>${label}</legend>
        <a href="javascript:;" id="${name}_check" onclick="$('input.${name}').attr('checked',true);reloadForm(null, this);">Check all</a>
        <a href="javascript:;" id="${name}_uncheck" onclick="$('input.${name}').removeAttr('checked');reloadForm(null, this)">Uncheck all</a><br/>
        %for key,value in opts.items():
...
<%def name="textarea(name, value = '', label = None, rows = 5, readonly = False, reload = False)">
...
<%def name="password(name, value='', label=None, reload=False)">
    <p><label>${label if label else name}<br>
