...
def checkDependencies():
    ...
    r = executeShellCmd('pkg-config cairo --atleast-version=1.8.10', pipe=False, onError='nothing')
    if r:
        print 'FAILED: Requires cairo >= 1.8.10'
        sys.exit(1)
    else:
    ...
    try:
        ver = pkg_resources.require('numpy >= 1.7.0b1')
        print 'OK: Numpy version >= 1.7.0b1 found: %s' % ver
    except:
        print 'FAILED: Requires Numpy version >= 1.7.0b1.'
        sys.exit(1)
        
    try:
        ver = pkg_resources.require('rpy2 >= 2.1.7')
        print 'OK: rpy2 version >= 2.1.7 found: %s' % ver
    except:
        print 'FAILED: Requires rpy2 version >= 2.1.7.'
        sys.exit(1)
        
    from gold.application.RSetup import getRVersion
    rVersion = getRVersion()
    if rVersion >= '2.13.2':
        print 'OK: R version >= 2.13.2 found: %s' % rVersion
    else:
        print 'FAILED: Requires R version >= 2.13.2. Version %s found.' % rVersion
...
def installAndCheckRLibraries():
    ...
    _installAndCheckRLibrary('gdata', '/usit/invitro/data/galaxy/eggs_repo/gdata_2.8.2.tar.gz')
    _installAndCheckRLibrary('gtools')
    _installAndCheckRLibrary('bitops')
    _installAndCheckRLibrary('caTools', '/usit/invitro/data/galaxy/eggs_repo/caTools_1.12.tar.gz')    
    _installAndCheckRLibrary('gplots')
    _installAndCheckRLibrary('maptree')
    _installAndCheckRLibrary('pi0', '/usit/invitro/data/galaxy/eggs_repo/pi0_1.2-139.tar.gz')
    
    import config.LocalOSConfig
    extraRLibraries = config.LocalOSConfig.__dict__.get('HB_EXTRA_R_LIBRARIES').strip().split()
    for library in extraRLibraries:
        _installAndCheckRLibrary(library)
...
def setupHB(galaxyPath='', ignoreVersionChecking=False, installationSetupFn=''):
    ...
    executePythonFile(os.sep.join(['setup', 'Install.py']), cwd=hbPath, setPythonPath=True)
