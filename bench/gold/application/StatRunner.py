...
from config.Config import PRINT_PROGRESS, MAX_LOCAL_RESULTS_IN_TABLE, USE_PROFILING, MAX_NUM_USER_BINS, USE_PARALLEL, DebugConfig
...
class StatJob(object):
    ...
    def run(self, reset=True, printProgress=PRINT_PROGRESS):        
        ...
        while True:
            ...
            if stat is None or stat.validateAndPossiblyResetGlobalResult(stat) == 0:
                break
...
class Progress():
    def __init__(self, totalCount, printProgress=True, description=None):
...
class SilentProgress(Progress):
    def __init__(self, totalCount, printProgress, description):
        Progress.__init__(self, totalCount, False, description)

class StandardProgress(Progress):
    #pass
    def __init__(self, totalCount, printProgress, description=None, numSamplesPerIteration=None):
...
class SequentialMcProgress(Progress):
    ...
    def __init__(self, totalCount, printProgress, description=None, numSamplesPerIteration=None):
        ...
        self._numSamplesPerIteration = numSamplesPerIteration if numSamplesPerIteration is not None else RandomizationManagerStatUnsplittable.NUM_SAMPLES_PER_CHUNK
