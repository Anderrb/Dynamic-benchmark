...
'''
        self._str += '\n'.join(['<script type="text/javascript" src="%s/static/scripts/%s"></script>' \
                                % (URL_PREFIX, javaScriptFn) for javaScriptFn in extraJavaScriptFns])
...
'''
        self._str += '\n'.join(['<link href="%s/static/style/%s" rel="stylesheet" type="text/css" />' \
                                % (URL_PREFIX, cssFn) for cssFn in extraCssFns])
...
<body>''' + os.linesep
    ...
    def tableHeader(self, headerRow, tagRow=None, firstRow=True, sortable=False):
        ...
        if firstRow:
            self._str += '<table class="colored bordered%s" width="100%%" style="table-layout:auto; word-wrap:break-word;">' \
                % (' sortable' if sortable else '') + os.linesep
        self._str += '<tr>'
