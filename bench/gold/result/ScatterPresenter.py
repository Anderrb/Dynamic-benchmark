...
class ScatterPresenter(LocalResultsGraphicsData, GraphicsPresenter):
    ...
    def _getRawData(self, resDictKey, avoidNAs=True):
        return [ [float(x[i]) for x in self._results.getAllValuesForResDictKey(resDictKey) if x is not None and not (avoidNAs and any(numpy.isnan(val) for val in x))] for i in [0,1] ]
