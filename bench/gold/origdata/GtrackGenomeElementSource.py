...
class GtrackGenomeElementSource(GenomeElementSource):
    ...
    VAL_TYPE_DICT = OrderedDict([('number', ValTypeInfo(pythonType=float, \
                                  ...
                                  ('category', ValTypeInfo(pythonType=str, \
                                                           ...
                                                           delim=',', \
                                                           missingVal='', \
                                                           fromNumpyTypeFunc=lambda t:t[0] == 'S' and t != 'S1'))])
    ...
    def _downloadSubtypeFileContents(cls, url):
        try:
            if url.startswith('http://gtrack.no/'):
                ...
                fn = os.sep.join([HB_SRC_STATIC_PATH, 'gtrack', url[17:]])
                return open(fn).read()
            else:
    ...
    def _checkDenseSorting(self, ge):
        if self._doDenseSortingCheck and self._prevElement is None or 'start' in self._columnSpec or not 'end' in self._columnSpec: 
            return
...
class HbGtrackGenomeElementSource(GtrackGenomeElementSource):
    ...
    def _appendBlankElement(self, chr, end=None):
        ...
        geCopy = copy(self._storedBlankElement)
