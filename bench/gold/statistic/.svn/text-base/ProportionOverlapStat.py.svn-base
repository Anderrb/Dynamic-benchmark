...
class ProportionOverlapStatUnsplittable(Statistic):
    def _compute(self):
        ...
        all = sum( res[key] for key in ['Neither','Only1','Only2','Both'] )

        res['NeitherProp'] = res['Neither']*1.0/all
        res['Only1Prop'] = res['Only1']*1.0/all
        res['Only2Prop'] = res['Only2']*1.0/all
        res['BothProp'] = res['Both']*1.0/all
