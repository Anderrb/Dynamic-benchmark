...
class BasicCustomRStatUnsplittable(Statistic):
    ...
    def _compute(self):
        ...
        for i in [0,1]:
            ...
            for index, el in enumerate(tv):
                if el.start() == el.end() == None:
                    ...
                    track[i] += [int(el.start()), int(el.end()), int(float(el.val())) if el.val() is not None else None]
        ...
        track[0] = r.unlist(track[0])
        #print "track 0 after unlist: ", track[0]
        track[1] = r.unlist(track[1])    
        #print "track 1 after unlist: ", track[1] 
        return r(allLines)(track[0], track[1])
