...
class TestMarksSortedBySegmentOverlapStatUnsplittable(StatUnitTest):
    classToCreate = MarksSortedBySegmentOverlapStat
    ...
    def test_compute(self):
        self._assertCompute( ([3,2,1],[0.8,0.6,0.5]), SampleTV( segments = [[10,20],[30,40],[50,55]], vals = [1,2,3], valDType='int32'),\
                             SampleTV(segments = [[15,25],[34,40],[51,55]]), markReq='number (integer)', \
                             assertFunc=self.assertListsOrDicts) 
        self._assertCompute(([],[]), SampleTV( numElements = 0, valDType='int32'),\
                            SampleTV(segments = [[15,25],[34,40],[51,55]]), markReq='number (integer)', \
                            assertFunc=self.assertListsOrDicts) 
