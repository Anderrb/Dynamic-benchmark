...
class TestMarksSortedByFunctionValueStatUnsplittable(StatUnitTest):
    classToCreate = MarksSortedByFunctionValueStat
    ...
    def test_compute(self):
        self._assertCompute(( [3,2,1], [52.0, 34.5, 14.5] ), SampleTV( segments = [[10,20],[30,40],[50,55]], vals = [1,2,3], \
                                                                       anchor = [0,100], valDType='int32'),\
                                     SampleTV_Num( vals = range(100), anchor = [0,100] ), markReq='number (integer)',\
                                     assertFunc=self.assertListsOrDicts)
        self._assertCompute(([],[]), SampleTV( numElements = 0, anchor = [0,100], valDType='int32'),\
                                     SampleTV_Num( vals = range(100),anchor = [0,100]), markReq='number (integer)',\
                                     assertFunc=self.assertListsOrDicts)
...
#    classToCreate = MarksSortedBySegmentOverlapStatStat
#
#    def test_compute(self):
#        pass
#        
