...
genome = 'hg18'
regulomes = [x.strip() for x in open('test/sandbox/aux/regulomes.txt')]
fixTns = dict([tuple([tuple(tn.strip().split(':')) for tn in x.strip().split('\t')]) for x in open('test/sandbox/aux/regulomeFixTns.txt')])
