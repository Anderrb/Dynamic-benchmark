...
from quick.aux.CustomFuncCatalog import parseMatrixTextFileToShelf, mergeShelvesTransitively, reverseMappingHavingListValues
from config.Config import RESULTS_FILES_PATH, STATIC_PATH
from quick.util.CommonFunctions import getUniqueWebPath
import getpass
...
#print self.CUR_DIR_ON_INSILICO
#print self.CUR_DIR_ON_MEDSEQPROD

class GoogleMapCreator():
    USERNAME = getpass.getuser()
    #todo: needs fix for generality
    HOME_DIR_ON_INSILICO = '/usit/titan/u1/' + USERNAME
    HOME_DIR_ON_MEDSEQPROD = '/insilico_home/' + USERNAME
    TMP_DIR_ON_INSILICO = HOME_DIR_ON_INSILICO + '/tmp'
    TMP_DIR_ON_MEDSEQPROD = HOME_DIR_ON_MEDSEQPROD + '/tmp'
    IMAGEMAGICK_TMP_DIR_ON_MEDSEQPROD = '/usit/med-seqprod01/data/sveinugu/tmp'
    
    CUR_DIR_ON_INSILICO = Popen(['pwd'], shell=True, stdout=PIPE).communicate()[0].strip()
    CUR_DIR_ON_MEDSEQPROD = CUR_DIR_ON_INSILICO.replace(HOME_DIR_ON_INSILICO, HOME_DIR_ON_MEDSEQPROD)

    def __init__(self, name, id, mapId, batchid=0):
        self._name = name
        self._mapId = mapId
        
        colldir =  ('%.3f' % (int(id)/1000 *1.0 / 1000))[2:]
        
        self._resultsFilesDir = '/'.join([RESULTS_FILES_PATH, colldir])
        self._resultsStaticDir = getUniqueWebPath([colldir, id, batchid])
    ...
    def tile(self, onMedSeqProd):
        databaseFile = open(self._resultsFilesDir + '/dataset_%s.dat' % id, 'r')
        
        for line in databaseFile:
            if line.startswith('('):
                imagesize = max([int(x) for x in line.replace('(','').replace(',','').replace(')','').strip().split()])
                
        shutil.copy('third_party/tileCutter3.py', '/'.join([self.TMP_DIR_ON_INSILICO, 'tileCutter3.py']))
    
        if not os.path.exists('/'.join([self.TMP_DIR_ON_INSILICO, self._name, 'data'])):
            os.makedirs('/'.join([self.TMP_DIR_ON_INSILICO, self._name, 'data']))
            
        shutil.copy(self._resultsStaticDir + '/ClusterMatrixStat_Result_heatmap.png', \
                    '/'.join([self.TMP_DIR_ON_INSILICO, self._name, 'data']))
    ...
    #    print Popen(['ssh','med-seqprod01.uio.no', 'cd', self.TMP_DIR_ON_MEDSEQPROD, ';', 'python', 'tileCutter3.py', outputDir, 'png', '/'.join([self._name, 'data', 'ClusterMatrixStat_Result_heatmap.png']), '0', '0', str(imagesize), str(imagesize), '0', '0', '0', '255', '255', '\"#111111\"'], stdout=PIPE).communicate()[0]
        if onMedSeqProd:
            ...
            os.chdir(self.TMP_DIR_ON_INSILICO)
            print Popen(['python', 'tileCutter3.py', outputDir, \
                         'png', '/'.join([self._name, 'data', 'ClusterMatrixStat_Result_heatmap.png']), \
                         '0', '0', str(imagesize), str(imagesize), \
                         '0', '0', '0', '255', '255', '#111111'], stdout=PIPE).communicate()[0]
    #    shutil.move(self.TMP_DIR_ON_INSILICO + '/t-images_%s' %id, '/'.join([self.TMP_DIR_ON_INSILICO, self._name, 'tiles']))
        shutil.move('/'.join([self.TMP_DIR_ON_INSILICO, self._name]), self._googleMapsDir)
    
    
    def copyWithoutTiling(self):
        if os.path.exists( '/'.join([self.TMP_DIR_ON_INSILICO, self._name]) ):
            shutil.move('/'.join([self.TMP_DIR_ON_INSILICO, self._name]), self._googleMapsDir)
        else:
    ...
    def copyResultFiles(self):
        ...
        if os.path.exists(self._resultsStaticDir + '/data/Result_counts_table.txt'):
        ...
        if os.path.exists(self._resultsStaticDir + '/data/Result_pval_table.txt'):
        ...
        if os.path.exists(self._resultsStaticDir + '/data/Result_significance_table.txt'):
    ...
    def createResultShelves(self):
        ...
        htmlFileContents = open(self._resultsFilesDir + '/dataset_%s.dat' % id, 'r').read()
        ...
        runDescFile.write(htmlFileContents[htmlFileContents.find("<div class=\"infomessagesmall\">"):
                                           htmlFileContents.find("<hr />\n<h3>Corresponding batch-run line</h3>")])
        runDescFile.close()
    ...
    def createIndexFile(self, title):
...
if __name__ == "__main__":
    if len(sys.argv) not in [8,9,10]:
        print 'Usage: python createGoogleMap.py id batchid name mapId doTiling doCreateResultShelves doCreateIndexFile title=name onMedSeqProd=False'
        sys.exit(0)
    ...
    creator = GoogleMapCreator(name, id, mapId, batchid)
