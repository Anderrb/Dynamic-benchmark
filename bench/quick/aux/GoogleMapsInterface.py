...
#        rowName = self.getRowName(lowerCase=True)
    ...
    def _exportTrackToFile(self, trackName, fn):
        GalaxyInterface.extractTrackManyBins('hg18', trackName, '*', '*', True, 'gtrack', True, True, fn)
    ...
    def _getExportTrackHref(self, rowOrCol='row'):
        ...
        return URL_PREFIX + '/hyper?mako=extract&track1=%s' % quote(track1)
    ...
    def _getSelectTracksHref(self, withRowTrack=False, withColTrack=False):
        ...
        return URL_PREFIX + '/hyper?track1=%s' % quote(track1) + ('&track2=%s' % quote(track2) if track2 != None else '')
    ...
    def _writeTrackForGeneListWithHits(self, fn):
        GalaxyInterface.getGeneTrackFromGeneList('hg18', self.getColTrackName(), \
    ...
    def getExportGeneListWithHitsLink(self):
        ...
        href = URL_PREFIX \
            + '/tool_runner?tool_id=import_genelist&runtool_btn=yes&dbkey=hg18&map=%s&col=%d&row=%d&mapid=%s' \
            % (self._mapName, self._colIdx, self._rowIdx, self._mapId)
