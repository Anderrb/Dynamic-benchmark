...
def getCategorySetForSubTracks(genome, baseTrackName, shelveFn):
    ...
    for trackName in OrigTrackNameSource(genome, baseTrackName):
        ...
        basePath = createOrigPath(genome, trackName)
...
def createShelvesBehindRankedGeneLists(galaxyId, mapId, countType):
    ...
    #hent txt
    resultsStaticDir = getUniqueWebPath([colldir, str(galaxyId), batchid])
    #legg shelf
...
def mergeSubtypeTracksToNewCategoryTrack(genome, trackName, newTrackName=None):
    ...
    basePath = createOrigPath(genome, trackName)
    newFn = createOrigPath(genome, newTrackName) + os.sep + 'merged.category.bed'

    categoryDict = {}
    ensurePathExists(newFn)
    outF = open(newFn,'w')
    for subType in os.listdir(basePath):
        subPath = basePath + os.sep + subType
        if not os.path.isdir(subPath):
...
def _countAnalyses(analyses):
    analyses = [a for a in analyses if len(a._analysisParts) > 0]
    print 'Number of analyses ignoring assumptions: ', len(analyses)
...
def createChromosomeFile(genome, chromNames, referToCollected=False):
    """genome chromNames"""
    # python quick/aux/CustomFuncCatalog.py CreateChromosomeFile mm9 'chr1, chr2, ...'"
    
    chrList = chromNames.replace(' ','').split(',')
    if referToCollected:
        basePath = createCollectedPath(genome, GenomeInfo.getPropertyTrackName(genome, 'chrs'))
    else:
        basePath = createOrigPath(genome, GenomeInfo.getPropertyTrackName(genome, 'chrs'))
...
def createAssemblyGapsFile(genome, assemblyChars='ACGTacgt'):
    ...
    basePath = createOrigPath(genome, GenomeInfo.getPropertyTrackName(genome, 'gaps'),'')
    ...
    if not anyGaps:
        outFile.write('\t'.join([GenomeInfo.getExtendedChrList(genome)[0], '0', '0']))
...
def copyAndFilterSubtypes(genome, trackName, newTrackName, filterFn):
    ...
    origPath, newPath = [createOrigPath(genome, tn,'') for tn in [trackName, newTrackName]]
...
def compareRegulomeWithDirectPubmedTfData(pValThresholdReg, pValThresholdTf, useRowCount):
    ...
    tfDir = createOrigPath('hg18', ['Private', 'disease' 'all diseases, hyperg., only TFs'])
