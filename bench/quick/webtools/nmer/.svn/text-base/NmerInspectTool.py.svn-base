...
class NmerInspectTool(GeneralGuiTool):
    ...
    @staticmethod
    def getToolName():
        return 'Inspect nmer frequency variation'

    @staticmethod
    def getInputBoxNames(prevChoices=None):
        "Returns a list of names for input boxes, implicitly also the number of input boxes to display on page. Each such box will call function getOptionsBoxK, where K is in the range of 1 to the number of boxes"
        return ['Genome build: ', 'Nmer (any length): ', 'Region of the genome: ']
    ...
    def getToolDescription():
        from gold.result.HtmlCore import HtmlCore
        core = HtmlCore()
        core.paragraph('Inspect the frequency variation of occurrences of a given nmer in a specified region along the selected genome. The tool tries to determine appropriate binning of the specified region, and outputs the number of Nmer occurrences per bin as a plot and as underlying textual data.')
        core.divider()
        core.highlight('Nmer')
