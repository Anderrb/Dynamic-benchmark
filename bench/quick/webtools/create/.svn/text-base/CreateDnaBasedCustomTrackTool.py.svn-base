...
class CreateDnaBasedCustomTrackTool(GeneralGuiTool):
    @staticmethod
    def getToolName():
        return "Create track from DNA sequence"
    ...
    def getToolDescription():
        ...
        core.divider()
        core.paragraph('''For each position in the selected genome, the supplied custom expression is called on the DNA sequence of a window of selected size around the region. Details on the expression format:''')
        ...
        core.divider()
        core.paragraph('''Example of custom expression, computing GC content in sliding window: <br>
"sum([g[i]+c[i] for i in winIndexes])''')
        return str(core)
