    ...
    def getInputBoxNames():
        ...
        return [('Select input source:', 'source'), \
                ...
                [x for x in chain(*((('Select the name for column #%s' % (i+1), 'column%s' % i), \
                                     ('Type in a custom name for column #%s' % (i+1), 'customColumn%s' % i)) for i in xrange(TabularToGtrackTool.NUM_COLUMN_FUNCTIONS)))] + \
...
#                    error = 'Error: the number of columns varies over the rows of the tabular file.'
    ...
    def getOptionsBoxColumnSelection(prevChoices):
        if prevChoices.history or prevChoices.input:
            return ['Select individual columns', 'Base columns on existing GTrack file']
    ...
    def getColumnList(cls, prevChoices, index):
        if (prevChoices.history or prevChoices.input) and prevChoices.columnSelection == 'Select individual columns':
            fileContentsInfo = TabularToGtrackTool._getFileContentsInfo(prevChoices)
            if fileContentsInfo.error is None and index < fileContentsInfo.numCols:
                return ['-- ignore --']+ \
                        [x for x in cls.DEFAULT_COLUMNS if x not in cls._getAllPrevColumnChoices(prevChoices, index)] + \
                         ['-- custom --']
