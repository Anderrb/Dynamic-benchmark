...
import quick.aux.StandardizeTrackFiles as StandTrackFiles
...
class StandardizeTrackFilesTool(GeneralGuiTool):
    ...
    def execute(choices, galaxyFn=None, username=''):
        ...
        extraParams = ['direction=' + StandardizeTrackFilesTool.DIRECTION_DICT[choices.direction].parseFileArg,\
                       'allSubTypes=' + choices.allSubTypes,\
                       'subTypeDepth=' + choices.subTypeDepth]
        
        if len(StandardizeTrackFilesTool._extraParams)>0:
            for i in range(len(StandardizeTrackFilesTool._extraParams)):
        ...
        print 'Running with these arguments: ' + str(arguments)
        
        StandTrackFiles.runParserClass(arguments, printUsageWhenError=False)
        
        try:
            if choices.file:
                filePathList = StandardizeTrackFilesTool._getFilePathList(choices, input=False)
                filePath = StandardizeTrackFilesTool._getFilePath(filePathList, choices)
                print ''
                print ''
